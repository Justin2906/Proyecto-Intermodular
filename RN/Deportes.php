...
$consultaSQL = "SELECT id, Nombre, Direccion, Foto from Lugares where Tipo = 'Deportes';";
$listaPolideportivos = array();

if ($resultado = mysqli_query($conexion, $consultaSQL)) {
    while ($row = mysqli_fetch_assoc($resultado)) {
        $listaPolideportivos[] = $row;
    }
}
?>
<!DOCTYPE html>
    ...
    <main>
        <div class="info" style="justify-content:center; display: flex; margin-bottom:5%; margin-top:10%">
            <div class="polis">
                ...
                <div class="services">
                    <?php
                    foreach ($listaPolideportivos as $registro) { ?>
                    <div class="info2">
                        <h3><?php print_r($registro['Nombre']); ?></h3>
                        <h4>📍<?php print_r($registro['Direccion']); ?></h4>
                        <img src="Fotos BBDD/<?php echo ($registro['Foto']) ?>" alt=""><br>
                        <a class="btn btn-primary" href="Instalaciones.php?id=<?php echo ($registro['id']) ?>">Reservar</a><br>
                    </div>&nbsp;&nbsp;&nbsp;
